...
router.get('/register', async (req, res) => {
    res.render('auth_templates/register', { errors: req.session.errors });//Renderizando o meu formulçário para o usuário
});

router.post('/register', async (req, res) => {
    req.session.errors = []; // Se houver erros de sessões, serão limpos antes de exibir outro

    const {name, email, password, confirm_password} = req.body; // Pegando as informações obtidas do formulário
    ...
    try {
        ...
        connection.query('INSERT INTO users (name, email, password ) VALUES (?, ?, ?)', [name, email, hashedPassword], async (err, results) => {
            if (err) {
...
router.get('/login', (req, res) => {
    res.render('auth_templates/login', { errors: req.session.errors});
})
...
router.post('/login', (req, res)=>{
    ...
    connection.query('SELECT * FROM users WHERE email = ?', [email], async (err, results) => {
        ...
        if(results.length === 0){
            req.session.erros.push('Credenciais inválidas. Por favor, verifique seus dados e tente novamente');
            return res.redirect('/auth/login');
        };
...
router.get('/forgot_password', (req, res) => {
    res.render('auth_templates/forgot_password', {errors: req.session.errors});
})
...
router.get('/reset_password/:token', async (req, res) => {
    ...
    connection.query('SELECT * FROM users WHERE reset_token = ?', [token], (err, results) => {
        ...
        res.render('auth_templates/reset_password', { token, errors: req.session.errors });
    });
});
...
router.get('/logout', (req, res) =>{
    req.session.destroy((error) => {
        if(error) {
            console.log('Erro ao deslogar do sistema', error);
        } else {
            res.redirect('/books')
        }
    })
